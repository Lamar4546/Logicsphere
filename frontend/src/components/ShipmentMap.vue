...
const hasLocation = computed(() => Number.isFinite(Number(props.shipment.current_latitude)) && Number.isFinite(Number(props.shipment.current_longitude)))
const etaLabel = computed(() => props.shipment.eta_current ? new Date(props.shipment.eta_current).toLocaleString() : 'ETA not provided')
function renderMap() {
  ...
  if (hasLocation.value) {
    marker = L.marker(coordinates.value, { icon: L.divIcon({ className: 'shipment-marker', html: '<span>⌁</span>', iconSize: [32, 32], iconAnchor: [16, 16] }) }).addTo(map).bindPopup(`<strong>${props.shipment.reference_number}</strong><br>ETA: ${etaLabel.value}`).openPopup()
...
<template>
  <aside class="panel map-card">
    ...
    <div class="map-frame"><div ref="mapElement" class="map-wrap" aria-label="OpenStreetMap live shipment location"></div><div class="eta-overlay"><span>Current ETA</span><strong>{{ etaLabel }}</strong><small :class="{ live: hasLocation }">{{ hasLocation ? '● Live position received' : '○ Awaiting position' }}</small></div></div>
